...
%token PLUS MINUS TIMES DIVIDE ASSIGN PIPE NOT POWER MOD
%token MATMUL MATTRANSPOSE MATDOTMUL
...
%left COLON
%left PLUS MINUS
%left TIMES DIVIDE MATMUL MATDOTMUL MOD
%left POWER
%right NOT NEG
%right MATTRANPOSE
...
model_stmt_pipe:
  model_stmt PIPE model_stmt { Pipe($1, $3) } */

model_stmt:
  model_expr { Expr $1 }
| LBRACE model_stmt_list RBRACE { Block(List.rev $2) }
| FOR LPAREN expr_opt SEMI expr SEMI expr_opt RPAREN model_stmt
                                                    { For($3, $5, $7, $9) }
...
| LBRACE gen_stmt_list RBRACE                       { Block(List.rev $2) }
